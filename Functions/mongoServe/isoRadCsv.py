...
def csvGen():
    ...
    __Reescribir(ruta=f'{__ruta}/Functions/isoyetas/isoRAD',planchar=True)
    for x in coords.keys():
        id=int(x)-1000
        total=__sumatoria(f"/datosSQL/{x}")
        try:
            utmx,utmy=__transformacion(coords.get(int(x))[3],coords.get(int(x))[4],14)
            __Reescribir(f'{__ruta}/Functions/isoyetas/isoRAD',id,total,utmx,utmy,coords.get(int(x))[3],coords.get(int(x))[4])
        except:
            continue
#csvGen()
#print(__ruta)